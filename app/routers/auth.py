...
def register(body: UserRegister, db: Session = Depends(get_db)):
  ...
  if existing:
    logger.warning(f'Registration attempt with existing email: {body.email}')
...
def login(body: UserLogin, db: Session = Depends(get_db)):
  ...
  if is_login_throttled(body.email):
    logger.warning(f'Login throttled for email: {body.email}')
    raise HTTPException(
      status_code=429,
  ...
  if not password_valid:
    record_login_attempt(body.email, False)
    raise HTTPException(status_code=401, detail='Credenciales inválidas')

  record_login_attempt(body.email, True)
  token = create_access_token({'sub': str(user.id)})
